...
sub print_module ($$$$;$$$$$$$) {
	...
	my $MODULE_UNIT = $_[5];
	my $MODULE_MIN_WARNING = $_[6];
	my $MODULE_MAX_WARNING = $_[7];
	my $MODULE_MIN_CRITICAL = $_[8];
	my $MODULE_MAX_CRITICAL = $_[9];
	...
	if (defined($MODULE_UNIT)) {
		print "<unit>$MODULE_UNIT</unit>\n";
	}
	
	if (defined($MODULE_MIN_WARNING)) {
		print "<min_warning>$MODULE_MIN_WARNING</min_warning>\n";
	}
	if (defined($MODULE_MAX_WARNING)) {
		print "<max_warning>$MODULE_MAX_WARNING</max_warning>\n";
	}
	if (defined($MODULE_MIN_CRITICAL)) {
		print "<min_critical>$MODULE_MIN_CRITICAL</min_critical>\n";
	}
	if (defined($MODULE_MAX_CRITICAL)) {
		print "<max_critical>$MODULE_MAX_CRITICAL</max_critical>\n";
	}
...
sub parse_config {
	...
	foreach (@config_file) {
		...
		if ($parametro =~ m/^check\_begin/i) {
			...
			$checks[$plugin_setup{"numchecks"}]{'unit'} = "";
		}
		...
		if ($check_block == 1) {
			...
			if ($parametro =~ m/^unit\s(.*)/i) {
				$checks[$plugin_setup{"numchecks"}]{'unit'} = trim($1);
			}
			
			
			
			if ($parametro =~ m/^min_warning\s(.*)/i) {
				$checks[$plugin_setup{"numchecks"}]{'min_warning'} = trim($1);
			}
			if ($parametro =~ m/^max_warning\s(.*)/i) {
				$checks[$plugin_setup{"numchecks"}]{'max_warning'} = trim($1);
			}
			if ($parametro =~ m/^min_critical\s(.*)/i) {
				$checks[$plugin_setup{"numchecks"}]{'min_critical'} = trim($1);
			}
			if ($parametro =~ m/^max_critical\s(.*)/i) {
				$checks[$plugin_setup{"numchecks"}]{'max_critical'} = trim($1);
			}
...
sub check_mysql_service {
	...
	$mysql_service_result = `ps -ef | grep mysql | wc -l`;
	if ($mysql_service_result > 1) {
...
sub check_mysql_memory {
	...
	if ($OS =~ /hpux/i){
		...
		$mysql_memory_result = `ps aux | grep -v "grep" | grep "mysqld" | awk '{print \$4}'`;
	}
...
sub check_mysql_cpu {
	...
	if ($OS =~ /hpux/i){
		...
		$mysql_cpu_result = `ps aux | grep -v "grep" | grep "mysqld" | awk '{print \$3}'`;
	}
...
sub check_system_diskusage {
	...
	my $system_diskusage_result_tmp = `df -P -k "$plugin_setup{'conf_mysql_homedir'}" | awk '{print \$5}' | tail -1 | tr -d "%"`;
 
	$system_diskusage_result = $system_diskusage_result_tmp;
...
sub check_mysql_ibdata1 {
	...
	if (!defined($plugin_setup{"conf_mysql_homedir"})) {
		logger("[INFO] check_mysql_ibdata1 check needs conf_mysql_homedir token defined in configuration file.");
	}
	
	$mysql_ibdata1_result = `du -k $plugin_setup{"conf_mysql_homedir"}/ibdata1 | tail -1 | awk '{ print \$1 }'`;
...
sub check_mysql_connection {
	...
	if ($mysql_connection_result == 0) {
		print_module(
			...
			"async_string",
			"Error",
			"MySQL plugin cannot connect to Database. Abort execution of plugin",
			"CRITICAL");
		print_module(
			"MySQL_mysql_service",
			"generic_proc",
...
if ( ! -f $archivo_cfg ) {
	...
	print_module(
		...
		"generic_proc",
		0,
		"Cannot open configuration file at $archivo_cfg, please set permissions correctly.");
...
foreach (@checks) {
	...
	my $unit = $_->{'unit'};
	my $min_warning = $_->{'min_warning'};
	my $max_warning = $_->{'max_warning'};
	my $min_critical = $_->{'min_critical'};
	my $max_critical = $_->{'max_critical'};
